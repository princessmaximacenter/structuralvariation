...
# Older code to infer cn change around complex sv similar as unb ctx approach ----

if(FALSE) {
  #2023-05-12/15 doesnt work as intended because yes fragment is lost but alos partially overlapping the complex sv footprint itself, but not always predicatble how and using before/after start/end of the footprint is not adequate for inferring whether there is underlying aneuploidy or not, eg becaus ethe footprint has oscillating CN state
  ## True positive HB 1p loss vs true negative cannot distinguish by these features
  
  
  complex_windows_chrom_df = get_split_range_by_breakpoint(svs = complex_footprints_gr[complex_footprints_gr$flag_is_complex],
                                                           patients_svs_df = complex_footprints %>% filter(flag_is_complex),
                                                           ranges=chromosomes,
                                                           ranges_df=chromosomes_df,
                                                           svs_id_col="complex_sv_footprint_id",
                                                           ranges_id_col="seqnames")
  complex_windows_chrom_df = complex_windows_chrom_df %>% left_join(complex_footprints[,c("complex_sv_footprint_id","patient_label")])
  
  complex_windows_chrom_df =  assess_cn_state_window_before_after(
    segments,segments_df,
    ctx_windows_chrom_df = complex_windows_chrom_df,
    svs_id_col="complex_sv_footprint_id",
    patient_id_col="patient_label")
  
  #backup
  complex_windows_chrom = complex_windows_chrom_df
  
  #for this analysis: single window option, and not interested in before/after per se but just if any cn change could be attributable to 
  ### hindsight: how do I infer CN change without use of before/after - could be noise otherwise
  ### use the same function but with flag
  
  complex_windows_chrom = complex_windows_chrom %>% annotate_stable_size_pass_window(unbalanced_ctx_window_width_min = 5e6,min_window_width_both = F,stable_windows_both = F)
  complex_windows_chrom = complex_windows_chrom %>% annotate_cn_change_window(cr_l2fc_threshold_sv_change = 0.2,apply_cr_value_minimum = T)
  complex_windows_chrom = complex_windows_chrom %>% dplyr::mutate(window_id_before=paste0(window_id_before,"_chrom"),window_id_after=paste0(window_id_after,"_chrom"))
  
  if(FALSE) {
    #tried this 2023-05-12 but dont need sep. function see above
    cr_l2fc_threshold_sv_change=0.2
    unbalanced_ctx_window_width_min=10e6
    
    annotate_cn_change_window_before_after = function(df,unbalanced_ctx_window_width_min = 10e6,cr_l2fc_threshold_sv_change = 0.2) {
      df = df %>% 
        dplyr::mutate(
          flag_cn_diff_window = abs(frac_cr_l2fc_50_before-frac_cr_l2fc_50_after) > cr_l2fc_threshold_sv_change,
          flag_cn_change_window_before = ifelse(frac_cr_stable_before & window_width_before > unbalanced_ctx_window_width_min,flag_cn_diff_window,NA ),
          flag_cn_change_window_after = ifelse(frac_cr_stable_after & window_width_after > unbalanced_ctx_window_width_min,flag_cn_diff_window,NA),
          feature_value_cn_change = 
            ifelse( (!is.na(flag_cn_change_window_before) & !is.na(flag_cn_change_window_after)),paste0("before: ",frac_call_before,", after: ",frac_call_after),
                    ifelse( (!is.na(flag_cn_change_window_before)),paste0("before: ",frac_call_before,", after: NA"),
                            ifelse( (!is.na(flag_cn_change_window_after)),paste0("before: NA, after: ",frac_call_after),NA))))
      return(df)
    }
    complex_windows_chrom = complex_windows_chrom %>% annotate_cn_change_window_before_after()
  }
  
  
  ## chr arm
  complex_windows_chr_arm_df = get_split_range_by_breakpoint(svs = complex_footprints_gr[complex_footprints_gr$flag_is_complex],
                                                             patients_svs_df = complex_footprints %>% filter(flag_is_complex),
                                                             ranges=chr_arms,
                                                             ranges_df=chr_arms_df,
                                                             svs_id_col="complex_sv_footprint_id",
                                                             ranges_id_col="chr_arm")
  complex_windows_chr_arm_df = complex_windows_chr_arm_df %>% left_join(complex_footprints[,c("complex_sv_footprint_id","patient_label")])
  
  complex_windows_chr_arm =  assess_cn_state_window_before_after(
    segments,segments_df,
    ctx_windows_chrom_df = complex_windows_chr_arm_df,
    svs_id_col="complex_sv_footprint_id",
    patient_id_col="patient_label")
  
  complex_windows_chr_arm = complex_windows_chr_arm %>% annotate_stable_size_pass_window(unbalanced_ctx_window_width_min= 5e6,min_window_width_both = F,stable_windows_both = F)
  complex_windows_chr_arm = complex_windows_chr_arm %>% annotate_cn_change_window(cr_l2fc_threshold_sv_change = 0.2,apply_cr_value_minimum = T)
  complex_windows_chr_arm = complex_windows_chr_arm %>% dplyr::mutate(window_id_before=paste0(window_id_before,"_chr_arm"),window_id_after=paste0(window_id_after,"_chr_arm"))
  
  
  
  ## also start/end windows.
  #true negative is  M147AAD_complex_4c6c34cfd292fac1506bd94b2000505d for the 1p loss is chrom arm change not around the breakpoint
  
  complex_footprints_as_bp_gr = get_range_as_bp_gr(complex_footprints_gr)
  complex_footprints_as_bp_gr$complex_sv_footprint_id_orientation = paste0(complex_footprints_as_bp_gr$complex_sv_footprint_id,"_",complex_footprints_as_bp_gr$sv_bp_orientation)
  names(complex_footprints_as_bp_gr)=complex_footprints_as_bp_gr$complex_sv_footprint_id_orientation 
  
  complex_footprints_as_bp = complex_footprints_as_bp_gr %>% as.data.frame()
  
  svs_5mb_windows = make_window_around_sv_bp(svs_gr=complex_footprints_as_bp_gr[complex_footprints_as_bp_gr$flag_is_complex],
                                             sv_bp_flank_window_size=5e6,
                                             svs_id_col="complex_sv_footprint_id_orientation")
  ...
  cn_around_sv_bp_5mb = cn_around_sv_bp_5mb %>% annotate_cn_change_window(cr_l2fc_threshold_sv_change = 0.2,apply_cr_value_minimum = T)
  ...
  cn_around_sv_bp_5mb %>% left_join(complex_footprints_as_bp[,c("complex_sv_footprint_id","complex_sv_footprint_id_orientation","sv_bp_orientation")]) %>% 
    left_join(complex_footprints %>% select(complex_sv_footprint_id,complex_sv_id,contains("class"),cancer_type,seqnames)) %>% 
    filter(cancer_type=="hepatoblastoma" & seqnames=="chr1") %>% 
    select(complex_sv_footprint_id,contains("feature_value"),contains("frac_call"),sv_bp_orientation) %>% unique() %>%
    pivot_wider(id_cols = "complex_sv_footprint_id",names_from = c("sv_bp_orientation"),
                values_from = c("window_5mb_frac_call_before","window_5mb_frac_call_after","window_5mb_feature_value_cn_change")) %>%
    select(complex_sv_footprint_id,window_5mb_feature_value_cn_change_start,window_5mb_feature_value_cn_change_end,window_5mb_frac_call_before_start,window_5mb_frac_call_after_start,window_5mb_frac_call_before_end,window_5mb_frac_call_after_end)
  #values_from = c("window_5mb_feature_value_before","window_5mb_feature_value_after","window_5mb_feature_value_cn_change")) %>% as.data.frame()
  
  
  #how to look for recurrent? by overlapping windows?  
  complex_windows_chrom %>% get_windows_gr_from_ctx_properties(location = "chrom")
  
  
  complex_windows_chrom %>% left_join(complex_footprints %>% select(complex_sv_footprint_id,complex_sv_id,contains("class"),cancer_type,seqnames)) %>% 
    filter(cancer_type=="hepatoblastoma" & seqnames=="chr1") 
  
  
  complex_windows_chr_arm %>% left_join(complex_footprints %>% select(complex_sv_footprint_id,complex_sv_id,contains("class"),cancer_type,seqnames)) %>% 
    filter(cancer_type=="hepatoblastoma" & seqnames=="chr1") %>% select(complex_sv_id,complex_sv_class,contains("feature_value"))
  
  #none of these approaches work as intended. Best way forward might be to just overlap terminal chrom gain/loss fractions with complex sv footprints

## CN fractions footprints 
complex_footprint_cn_fractions = get_chrom_cn_fractions(segments,segments_df,
                                                                  ranges=complex_footprints_gr,
                                                                  ranges_df=complex_footprints,
                                                                  cohort,return_wide = T,
                                                                  ranges_id_col="complex_sv_footprint_id",ranges_width_col="footprint_width",relative_cr = F)

complex_footprint_cn_fractions %>% 
  left_join(complex_footprints %>% select(complex_sv_footprint_id,complex_sv_id,cancer_type,complex_sv_class_super),by="complex_sv_footprint_id") %>% 
#  filter(cancer_type=="hepatoblastoma" & grepl("chr1",complex_sv_footprint_id)) %>% unique()
#"there is a substantial fraction in loss state"
  filter(cancer_type=="nephroblastoma" & grepl("chr11",complex_sv_footprint_id)) %>% unique() %>% View()
#copy neutral footprints? no not really

complex_footprint_cn_fractions %>% 
  left_join(complex_footprints %>% select(complex_sv_footprint_id,complex_sv_id,cancer_type,complex_sv_class_super),by="complex_sv_footprint_id") %>% 
  filter(call=="0") %>% unique() 
...
if(FALSE) {
  #multisample
  multisample_path = paste0(metadata_dir,"overview_additional_samples.solids_v2.5.20230118.wgs.tsv")
  cohort_multisample = read.table(multisample_path,sep = "\t", header=T) 
  cohort_multisample = cohort_multisample %>% left_join(cohort[,c("patient_label","cancer_type","basename")] %>% dplyr::rename(patient_label_original=patient_label,basename_original=basename))
  cohort_multisample$patient_id=cohort_multisample$patient_label
  cohort_multisample$basename=paste0(cohort_multisample$tumor_id,"_",cohort_multisample$normal_id)
  
  unfiltered_svs_df = load_cohort_svs(cohort_multisample,c('${merged_svs_dir}'=merged_svs_dir),svs_path_template=svs_union_anno_somatic_multitool_path_template,flag_add_patient_label = T)
  unfiltered_svs_df = unfiltered_svs_df %>% annotate_sv_cytoband(chromosome_bands)%>% annotate_sv_chr_arm(chr_arms)
  unfiltered_svs_df = unfiltered_svs_df %>% rowwise() %>% mutate(partner_chrom = unlist(strsplit(partner_coordinate,":"))[1][]) %>% as.data.frame()
  unfiltered_svs_df = annotate_sv_multitool_support(unfiltered_svs_df,sv_tumor_af_threshold = 0.1)
  unfiltered_svs_df = unfiltered_svs_df %>% unique()
  merged_svs = make_merged_svs(unfiltered_svs_df)
  merged_svs = merged_svs  %>% left_join(cohort_multisample[,c("patient_label_original",patient_tumor_id_cols)])  
  merged_svs[is.na(merged_svs$patient_label_original),c("patient_label_original")]="M911AAA"
  
  merged_svs = merged_svs %>% filter(flag_in_filtered_svs) 
  #sets globals therefore no output
  resolve_orphan_svs(merged_svs,unfiltered_svs_df)
  ...
  #made the clusters using fn above
  complex_svs_results_dir = "/Users/ianthevanbelzen/results/solids_v4_multisample/complex_sv_clusters/"
  complex_sv_bp_cluster_mapping_path = paste0(complex_svs_results_dir,"complex_sv_bp_cluster_mapping.tsv")
  #write.table(complex_sv_bp_cluster_mapping ,complex_sv_bp_cluster_mapping_path,quote=F,sep="\t",col.names = T,row.names = F)
  ...
  cohort_merged_classes = read.table(paste0("~/results/solids_v4/","merged_svs_classes.",run_date,".tsv"),sep="\t",header = T)
  cohort_merged_classes = cohort_merged_classes %>% filter(patient_label %in% merged_svs_gr$patient_label_original)
  cohort_merged_classes_gr = GRanges(cohort_merged_classes$sv_merged_coordinate)
  mcols(cohort_merged_classes_gr) = cohort_merged_classes %>% select(-seqnames,-start,-end)
  names(cohort_merged_classes_gr) = cohort_merged_classes_gr$patient_sv_merged
  
  map_multisample_to_cohort = get_reciprocal_overlap_pairs(
    resize_gr_distance(cohort_merged_classes_gr,distance = 50),
    resize_gr_distance(merged_svs_gr,distance = 50),reciprocal_overlap = 0,svtype_matching = F)
  
  map_multisample_to_cohort = map_multisample_to_cohort  %>% select(-to,-from,-set1_svtype,-set2_svtype) %>%
    dplyr::rename(patient_sv_merged=set1) %>% 
    left_join(cohort_merged_classes %>% select(patient_sv_merged,svtype,tumor_af_mean,svlen_mean,contains("complex_"),flag_is_complex,patient_label,cancer_type)) %>%
    dplyr::rename(multisample_patient_sv_merged=set2) %>% 
    left_join(merged_svs %>% select(patient_sv_merged,svtype,tumor_af_mean,svlen_mean,contains("complex_"),patient_label_original) %>% dplyr::rename_with(.cols=-patient_label_original,function(x){paste0("multisample_",x)}),
              by=c("multisample_patient_sv_merged")) %>%
    filter(patient_label==patient_label_original)
  
  #write.table(map_multisample_to_cohort ,paste0(complex_svs_results_dir,"map_multisample_to_cohort.tsv"),quote=F,sep="\t",col.names = T,row.names = F)
  
  
  cohort_merged_classes %>% filter(tumor_af_mean > 0.3) %>% filter(!patient_sv_merged %in% map_multisample_to_cohort$patient_sv_merged) %>% 
    select(patient_sv_merged,complex_sv_class,seqnames,svtype)
  
  if(FALSE) {
    #annotate
    #load genes and annot with flags
    gene_df_cols = c("gene_id","gene_name","gene_coord","gene_type","seqnames",gene_properties_df_flags,"flag_pmc_panel","flag_gene_of_interest")
    
    overlaps_genes_svs_1mb = get_reciprocal_overlap_pairs(genes_1mb,merged_sv_bp_gr,ignore_strand = T,svtype_matching = F,reciprocal_overlap=0) %>%
      dplyr::rename(gene_id=set1, patient_sv_merged = set2 ) %>% 
      left_join(gene_properties_df[,gene_df_cols] %>% dplyr::rename(gene_chrom=seqnames)) %>%
      left_join(merged_svs %>% dplyr::select(patient_sv_merged,"svtype","tumor_af_mean",contains("complex"),patient_label))
  ...
  merged_svs_annotated = merged_svs %>% 
    dplyr::mutate(overlap_primary = patient_sv_merged %in% map_multisample_to_cohort$multisample_patient_sv_merged) %>% 
    left_join(overlaps_genes_svs_1mb %>% filter(flag_gene_of_interest) %>% 
      group_by(patient_sv_merged) %>% summarize(gene_1mb_lst=lst_str(gene_name))) %>%
    left_join(overlaps_genes_svs %>% filter(flag_gene_of_interest) %>% 
                group_by(patient_sv_merged) %>% summarize(gene_lst=lst_str(gene_name))) 
  
  merged_svs_annotated %>% filter(tumor_af_mean > 0.3) %>%
    select(patient_sv_merged,overlap_primary,tumor_af_mean,complex_sv_id,seqnames,svtype,gene_1mb_lst,gene_lst) %>% arrange(patient_sv_merged) %>% View()

  #save annotated
  #write.table(merged_svs_annotated ,paste0(complex_svs_results_dir,"merged_svs_annotated.tsv"),quote=F,sep="\t",col.names = T,row.names = F)
  ...
  complex_footprints = merged_svs %>% filter(!is.na(complex_sv_footprint_id)) %>% 
    group_by(patient_label,patient_label_original,complex_sv_id,complex_sv_footprint_id) %>% 
    summarize(start=min(start),end=max(end),seqnames=lst_str(seqnames),
              footprint_width=end-start,
              footprint_width_mbp = paste0(round(footprint_width/1e6,2)," Mbp")) %>% 
    get_gr_coordinate(attr_name = "complex_sv_local_footprint_coordinate") %>% ungroup()
  ...
  cohort_complex_footprints = cohort_merged_classes %>% filter(!is.na(complex_sv_footprint_id)) %>% 
    group_by(patient_label,complex_sv_id,complex_sv_footprint_id,complex_sv_class,complex_sv_class_super) %>% 
    summarize(start=min(start),end=max(end),seqnames=lst_str(seqnames),
              footprint_width=end-start,
              footprint_width_mbp = paste0(round(footprint_width/1e6,2)," Mbp")) %>% 
    get_gr_coordinate(attr_name = "complex_sv_local_footprint_coordinate") %>% ungroup()
  
  cohort_complex_footprints_gr = GRanges(cohort_complex_footprints$complex_sv_local_footprint_coordinate)
  names(cohort_complex_footprints_gr) = cohort_complex_footprints$complex_sv_footprint_id
  cohort_complex_footprints_gr$complex_sv_footprint_id = cohort_complex_footprints$complex_sv_footprint_id
  
  map_multisample_to_cohort_footprints = get_reciprocal_overlap_pairs(
    resize_gr_distance(cohort_complex_footprints_gr,distance = 1e3),
    resize_gr_distance(complex_footprints_gr,distance = 1e3),reciprocal_overlap = 0,svtype_matching = F)
  
  map_multisample_to_cohort_footprints = map_multisample_to_cohort_footprints  %>% select(-to,-from) %>%
    dplyr::rename(complex_sv_footprint_id=set1) %>% 
    left_join(cohort_complex_footprints %>% select(complex_sv_footprint_id,contains("complex_"),patient_label)) %>%
    dplyr::rename(multisample_complex_sv_footprint_id=set2) %>% 
    left_join(complex_footprints %>% select(complex_sv_footprint_id,contains("complex_"),patient_label_original) %>% dplyr::rename_with(.cols=-patient_label_original,function(x){paste0("multisample_",x)}),
              by=c("multisample_complex_sv_footprint_id")) %>%
    filter(patient_label==patient_label_original)
  
  #write.table(map_multisample_to_cohort_footprints ,paste0(complex_svs_results_dir,"map_multisample_to_cohort_footprints.tsv"),quote=F,sep="\t",col.names = T,row.names = F)
  ...
  if(FALSE) {
    ...
    overlaps_genes_complex_svs_footprints = get_reciprocal_overlap_pairs(gene_properties,complex_footprints_gr,ignore_strand = T,svtype_matching = F,reciprocal_overlap=0) %>%
      dplyr::rename(gene_id=set1, complex_sv_footprint_id = set2 ) %>%
      left_join(gene_properties_df[,gene_df_cols] %>% dplyr::rename(gene_chrom=seqnames)) %>%
      left_join(complex_footprints[,c("complex_sv_footprint_id","complex_sv_id","complex_sv_local_footprint_coordinate","patient_label")])
   
    
    complex_svs_local_footprints_cancer_genes = overlaps_genes_complex_svs_footprints %>%
      filter(flag_gene_of_interest) %>% group_by(complex_sv_id,complex_sv_footprint_id) %>%
      summarize(gene_lst_footprints=lst_str(gene_name))

  }
  complex_footprints_annotated = complexx_footprints %>% 
    dplyr::mutate(overlap_primary = complex_sv_footprint_id %in% map_multisample_to_cohort_footprints$multisample_complex_sv_footprint_id) %>% 
    left_join(complex_svs_local_footprints_cancer_genes) 
  
  #save annotated
  #write.table(complex_footprints_annotated ,paste0(complex_svs_results_dir,"complex_footprints_annotated.tsv"),quote=F,sep="\t",col.names = T,row.names = F)
  ...
  #the important amplicons are found back! in M154AAB M911AAA M947AAA
  map_multisample_to_cohort_footprints %>% filter(complex_sv_class=="amplicon")
  
  #not found back
  cohort_complex_footprints  %>% 
    filter(!complex_sv_footprint_id %in% map_multisample_to_cohort_footprints$complex_sv_footprint_id)  %>%
    select(complex_sv_footprint_id,seqnames,complex_sv_class_super) %>% filter(complex_sv_class_super!="simple")
  
  #did expect M263AAB_complex_3e1c5aba057d4aaf5e7478a650402fb0 to be maybe relevant  
  #gains/losses are still there, but ctx only 11,17 found 
  #merged_svs %>% filter(patient_label_original=="M263AAB") %>% filter(svtype=="CTX")
  
  complex_footprints_annotated  %>% 
    filter(overlap_primary==F) %>% select(complex_sv_footprint_id,gene_lst_footprints)
  
  cohort_complex_footprints  %>% 
    filter(complex_sv_footprint_id %in% map_multisample_to_cohort_footprints$complex_sv_footprint_id)  %>%
    filter(patient_label=="M947AAA") %>%
    select(complex_sv_footprint_id,seqnames,complex_sv_class_super) %>% filter(complex_sv_class_super!="simple") 
  
  #additional
  complex_footprints_annotated  %>% 
    filter(overlap_primary==F) %>% 
    select(complex_sv_footprint_id,overlap_primary,seqnames,gene_lst_footprints)
  
  
  complex_sv_stats %>% filter(complex_sv_id=="M947AAA_PMABM000HCT_complex_8b95ca50a2f618257711782be6aa7025") %>% as.data.frame()
  complex_sv_stats %>% filter(grepl("M947AAA",patient_label) & chrom_cnt>1) %>% as.data.frame()#=="M947AAA_PMABM000HCT_complex_8b95ca50a2f618257711782be6aa7025") %>% as.data.frame()
  
  complex_footprints_annotated  %>% 
    filter(patient_label_original=="M947AAA" & seqnames %in% c("chr1","chr2","chr13")) %>%
    select(complex_sv_footprint_id,overlap_primary,seqnames,gene_lst_footprints)
  
  cohort_merged_classes %>% filter(grepl("M947AAA",patient_label) & chrom_cnt>1) 
